...
export default function TrendSection({ storeId }: { storeId?: string }) {
  ...
  const handleAnalyzeStart = async () => {
    ...
    try {
      // 🔥 [핵심 수정 1] 서버가 원하는 'keyword' 이름으로 보냄
      const res = await fetch('/api/trend', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          keyword: selectedTrend.title, // trendData -> keyword 로 변경!
      ...
      // 🔥 [핵심 수정 2] 서버가 주는 'concepts'도 받을 수 있게 수정 (호환성 확보)
      const strategiesData = result.strategies || result.concepts;

      if (strategiesData && strategiesData.length > 0) {
        setStrategies(strategiesData);
        setStep('RESULT'); 
      } else {
        console.error("서버 응답:", result); // 에러 확인용 로그
        alert("전략 수립에 실패했습니다. (서버 응답 형식 불일치)");
        setStep('INPUT');
      }
    } catch (e) {
      console.error(e);
  ...
  const handleCreateBlog = async (strategy: any) => {
    // strategy 객체의 구조가 다를 수 있으므로 안전하게 필드 매핑
    // concepts로 올 경우 title, description, hook_message 필드를 가짐
    const topic = strategy.title;
    const concept = strategy.content || strategy.description; // content가 없으면 description 사용

    if (!confirm(`'${topic}' 전략으로 글을 작성하시겠습니까?`)) return;
                ...
                <div className="flex flex-col items-center justify-center py-10 space-y-6">
                  ...
                  <div className="w-full max-w-md">
                    ...
                    <input 
                      ...
                      onChange={(e) => setTargetStoreInput(e.target.value)}
                      className="w-full p-4 text-lg border-2 border-slate-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all"
                      autoFocus
                    />
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full max-w-md bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold text-lg py-4 rounded-xl shadow-lg hover:scale-[1.02] transition-transform flex items-center justify-center gap-2"
                  >
                    🚀 전략 분석 시작하기
                  </button>
                </div>
                  ...
                  {strategies.map((strategy, idx) => (
                    <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-200 hover:border-blue-500 hover:shadow-xl transition-all flex flex-col h-full relative group">
                      ...
                      <div className="flex-1 overflow-y-auto max-h-[200px] mb-4 pr-2 custom-scrollbar">
                        {/* content가 없으면 description을 보여주도록 처리 */}
